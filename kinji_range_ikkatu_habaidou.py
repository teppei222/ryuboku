...
with col1:
    ...
    if xy_xlsx is not None:
       ...
       #上幅の左右を出力
        ...
        first_delta = st.slider('底幅の左岸移動',-30.0,30.0,0.0,step=0.1)
        second_delta = st.slider('底幅の右岸移動',-30.0,30.0,0.0,step=0.1)
        opti.iloc[0,0] =opti.iloc[0,0] +hanpa+first_delta
        opti.iloc[-1,0]=opti.iloc[-1,0]-hanpa+second_delta
        opti['Y']=under_line
        saitekika = pd.concat([upper_cross,opti]).sort_values('X',ascending=True)
        y_value = high
        x1_value = saitekika.iloc[1,0]-saitekika.iloc[0,0]
        x2_value = saitekika.iloc[3,0]-saitekika.iloc[2,0]
        koubai_left = x1_value / high
        koubai_right = x2_value / high
        saitekika = pd.concat([saitekika,start_daikei])
        keisan_area = PolyArea(saitekika['X'],saitekika['Y'])

        st.write("上幅:" + str(round(joutei,2))+'m'+'  '+"下幅:"+str(round(x.x,2))+'m')
        st.write("高さ:" + str(round(high,2))+'m'+'  '+"最深河床高:"+ str(round(under_line,2))+'m')
        st.write('左岸法勾配:'+'1:'+str(round(koubai_left,1))+'  '+'右岸法勾配:'+'1:'+str(round(koubai_right,1)))

        river_shogen = []
        river_shogen.append(option)
        river_shogen.append(keisan_area)
        river_shogen.append(joutei)
        river_shogen.append(high)
        river_shogen.append(koubai_left)
        river_shogen.append(koubai_right)
        river_shogen.append(x.x) 
        river_shogen.append(under_line)
        river_shogen_df = pd.DataFrame([river_shogen],columns=['No','面積','上幅','高さ','左岸法勾配','右岸法勾配','下幅','最深河床高'])
        

with col2:
    if xy_xlsx is not None:
        st.write('下の二つが同一にならない場合は、横断の切断位置が変な場所になっている。')
        st.write('元の断面積：' + str(round(PolyArea(df_area['X'],df_area['Y']),2))+'m2')
        st.write('改変後面積：'+str(round(PolyArea(saitekika['X'],saitekika['Y']),2))+'m2')
        ...
        if answer == True:
            ...
            im_croped_screenshot = croped_screenshot.crop((10,195,1928,1058))
            im_croped_screenshot.save('./image/' + filename_image)
            st.write('キャプチャ終了')
        else:
            pass
